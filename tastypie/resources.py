    ...
    def __new__(cls, name, bases, attrs):
        ...
        if getattr(new_class._meta, 'include_absolute_url', True):
            if 'absolute_url' not in new_class.base_fields:
                new_class.base_fields['absolute_url'] = fields.CharField(attribute='get_absolute_url', readonly_post=True, readonly_patch=True)
        ...
        for identifier in kwargs:
            ...
            if bundle.request.method == 'POST':
                ...
                raise RuntimeError("lookup_kwargs_with_identifiers() was "
                        "called on a request that is not POST or PATCH; "
                        "This should never happen.")
            if readonly or field_object.is_related or\
